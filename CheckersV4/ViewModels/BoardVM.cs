...
namespace CheckersV4.ViewModels
{
    public class BoardVM : BaseVM
    {
        ...
        private static int[,] initialLineupNorth = {
            { 0, 1 }, { 0, 3 }, { 0, 5 }, { 0, 7 },
            { 1, 0 }, { 1, 2 }, { 1, 4 }, { 1, 6 },
            { 2, 1 }, { 2, 3 }, { 2, 5 }, { 2, 7 }
        };

        private static int[,] initialLineupSouth = {
            { 5, 0 }, { 5, 2 }, { 5, 4 }, { 5, 6 },
            { 6, 1 }, { 6, 3 }, { 6, 5 }, { 6, 7 },
            { 7, 0 }, { 7, 2 }, { 7, 4 }, { 7, 6 },
        };
        ...
        public void LoadInitialLineup()
        {
            ...
            for (int i = 0; i < initialLineupNorth.GetLength(0); i++)
            {
                int x = initialLineupNorth[i, 0];
                int y = initialLineupNorth[i, 1];

                Pieces.Add(new PieceVM(new Models.Piece(Models.Piece.Color.WHITE, new Location(x, y))));
            }

            for (int i = 0; i < initialLineupSouth.GetLength(0); i++)
            {
                int x = initialLineupSouth[i, 0];
                int y = initialLineupSouth[i, 1];
